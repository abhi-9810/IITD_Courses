...
void inorder(struct Node* node){
    ...
    inorder(node->left);
    for(int i=0;i<dimension;i++)
      cout<<(node->data)[i]<<"   "<<(node->mbr_left)[i]<<"  "<<(node->mbr_right)[i]<<" Data "<<endl;
...
void knn(struct Node * root,double query1[]){
    ...
    while(!min_heap.empty()){
        ...
        //cout<<left1<<"   "<<right1<<endl<<endl<<endl;
        //cout<<double( clock () - begin_time1 ) /  CLOCKS_PER_SEC<<"qwerty"<<endl<<endl;
        //begin_time1 = clock();
        if(q->left!=NULL)
          q->left->dist=left1;
        if(q->right!=NULL)
       ...
       // cout<<double( clock () - begin_time1 ) /  CLOCKS_PER_SEC<<"qwerty1"<<endl<<endl;
    }
}
...
int main(){
    ...
    //cout<<"data for KD TREE "<<dimension<<endl;
    ...
    while(i<q){
       ...
       for(int i1=l11-1;i1>=0;i1--){
           for(int i2=0;i2<dimension;i2++){
               myfile2<<points1[i1][i2]<<" ";
           }
           myfile2<<"\n";
       }
       seq+=double( clock () - begin_time ) /  CLOCKS_PER_SEC; 
       clock_t begin_time1 = clock();
       knn(root,query);
       ...
       knn1+=double( clock () - begin_time1 ) /  CLOCKS_PER_SEC;
    }
    ofstream myfile1;
    myfile1.open("average_dist.txt", std::ios_base::app); 
    cout<<"Sequential Time\n"<<seq<<endl<<endl;
    cout<<"Knn Time\n"<<knn1<<endl<<endl;
    myfile1<<dimension<<"   "<<(sum2/100.0)<<"    "<<(sum100/100.0)<<"\n";
    infile.close(); 
    myfile.close();
    myfile1.close();
}
