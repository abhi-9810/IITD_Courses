"""Improvement Changes in the implementation:
1. In this improvement we have used a dictionary to store children rather than a list as searching in dict is O(1) compared to O(n) in list
2. We also changed the header table it now contains both the head and the tail of the node links. So it decreases tree traversal upto a large extent.(Earlier we had to
   go through the whole list to get the tail.)
""" 
"""
The program gets the min_sup and min_conf from the user in runtime only but the filename is entered beforehand.
In order to change the file from where you want to get the data just edit the filename variable in main function.(use csv or .dat file)
"""
import itertools
...
class FPTree(object):
    ...
    def insert_transaction(self, items, node, headers):
        ...
        if child is not None:
            ...
            if headers[start] is None:
                headers[start] = [child, child]
            else:
                end = headers[start][1]
                end.link = child
                headers[start][1]=child
    ...
    def check_no_branching(self, node):
        ...
        if num_of_children > 1:
            ...
            return True and self.check_no_branching(node.children[list(node.children.keys())[0]])
